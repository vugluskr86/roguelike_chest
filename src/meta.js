...
export function defaultMeta(){ return { bestFloor:0, runs:0, totalCaptures:0, shards:0,
  upgrades:{ startSlots:0, startRelics:0, headstart:0 },
  codex:{ enemies:{}, relics:{}, curses:{}, kills:{} }, achievements:{} }; }
export let META = defaultMeta();
export function metaLoad(){
  try{
    const raw = window.localStorage && localStorage.getItem(META_KEY);
    if(raw){ const o=JSON.parse(raw); const d=defaultMeta(); META=Object.assign(d,o);
      META.upgrades=Object.assign(d.upgrades, o.upgrades||{});
      const c=o.codex||{};
      META.codex={ enemies:c.enemies||{}, relics:c.relics||{}, curses:c.curses||{}, kills:c.kills||{} };
      META.achievements=o.achievements||{}; }
  }catch(e){ /* хранилище недоступно (напр. песочница) — играем без персистентности */ }
}
export function metaSave(){ try{ if(window.localStorage) localStorage.setItem(META_KEY, JSON.stringify(META)); }catch(e){} }
export function upgradeCost(id){ const u=META_UPGRADES[id], lvl=META.upgrades[id]||0; return lvl>=u.max?null:u.costs[lvl]; }
export function buyUpgrade(id){
  const cost=upgradeCost(id);
  if(cost==null || META.shards<cost) return false;
  META.shards-=cost; META.upgrades[id]=(META.upgrades[id]||0)+1; metaSave(); return true;
}

export function codexSeeEnemy(t){ if(!META.codex.enemies[t]){ META.codex.enemies[t]=true; metaSave();
  if(BESTIARY_TRIO.every(id=>META.codex.enemies[id])) unlockAch('bestiary'); } }
export function codexSeeRelic(id){ if(!META.codex.relics[id]){ META.codex.relics[id]=true; metaSave(); } }
export function codexSeeCurse(id){ if(!META.codex.curses[id]){ META.codex.curses[id]=true; metaSave(); } }
export function recordKill(t, byPoison){ META.codex.kills[t]=(META.codex.kills[t]||0)+1; metaSave();
...
export function unlockAch(id){
  ...
  META.achievements[id]=true; metaSave();
...
export function endRunMeta(){
  ...
  META.shards+=earned; metaSave();
  if(META.shards>=100) unlockAch('wealthy');
  return earned;
}
export function codexProgress(){
  const allE=['pawn','knight','bishop','rook','queen','guardian','necro','mimic','assassin','priest','frost'];
  const total=allE.length+Object.keys(RELICS).length+Object.keys(CURSES).length;
  const have=allE.filter(t=>META.codex.enemies[t]).length
