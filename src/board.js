...
export function floodReach(wset, start){
  ...
  while(q.length){ const c=q.pop();
    for(const [dx,dy] of ORTHO){ const x=c.x+dx,y=c.y+dy;
      if(inB(x,y)&&!wset.has(key(x,y))&&!seen.has(key(x,y))){ seen.add(key(x,y)); q.push({x,y}); } }
  }
  return seen;
}
...
export function generateRoom(){
  ...
  const canWall=(x,y)=> x>0&&x<CFG.W-1&&y>0&&y<CFG.H-1 && !(x===start.x&&y>=CFG.H-2);
  if(style==='corridors'){
    ...
    for(const y of rows){
      const gaps=new Set(); const ng=1+randInt(2);
    ...
    const target=10+randInt(7); let guard=0;
    while(w.size<target && guard++<400){
      const x=1+randInt(CFG.W-2), y=1+randInt(CFG.H-3);
    ...
    const target=5+randInt(5); let guard=0;
    while(w.size<target && guard++<300){
      const x=1+randInt(CFG.W-2), y=1+randInt(CFG.H-3);
      if(!canWall(x,y)||w.has(key(x,y))) continue;
      w.add(key(x,y));
      if(Math.random()<0.5){ const [dx,dy]=pick(ORTHO), nx=x+dx, ny=y+dy;
...
export function placeSpecials(wset, reach, start){
  const sp=new Map();
  const cells=[...reach].map(k=>{ const [x,y]=k.split(',').map(Number); return {x,y,k}; })
    .filter(c=> c.x>0 && c.x<CFG.W-1 && c.y>0 && c.y<CFG.H-1 && !(c.x===start.x&&c.y===start.y));
  shuffle(cells);
  let i=0; const take=()=> i<cells.length ? cells[i++] : null;
  const fav = t => (S.biome&&S.biome.favorTiles||[]).includes(t) ? 0.25 : 0;   // уклон биома
  const nTrap = 1 + randInt(3);                         // 1–3 ловушки
  for(let t=0;t<nTrap;t++){ const c=take(); if(c) sp.set(c.k,{type:'trap'}); }
  if(Math.random()<0.6+fav('rune')){ const c=take(); if(c) sp.set(c.k,{type:'rune'}); }   // руна перезарядки
  if(Math.random()<0.5+fav('ice')){ const n=1+randInt(2); for(let i=0;i<n;i++){ const c=take(); if(c) sp.set(c.k,{type:'ice'}); } }  // лёд оглушает
  if(Math.random()<0.5+fav('portal')){ const a=take(), b=take();      // пара порталов
    if(a&&b){ sp.set(a.k,{type:'portal',pair:{x:b.x,y:b.y}}); sp.set(b.k,{type:'portal',pair:{x:a.x,y:a.y}}); } }
  // туман — облако из 2–4 клеток (скрывает угрозу)
  if(Math.random()<0.5+fav('fog')){ const n=2+randInt(3); for(let t=0;t<n;t++){ const c=take(); if(c) sp.set(c.k,{type:'fog'}); } }
  // конвейер — 1–2 клетки со стрелкой (сдвигает после хода)
  if(Math.random()<0.45+fav('conveyor')){ const n=1+randInt(2); for(let t=0;t<n;t++){ const c=take(); if(c) sp.set(c.k,{type:'conveyor',dir:pick(ORTHO)}); } }
  // цветовая зона — 1–2 клетки (только слон)
  if(Math.random()<0.4+fav('colorzone')){ const n=1+randInt(2); for(let t=0;t<n;t++){ const c=take(); if(c) sp.set(c.k,{type:'colorzone'}); } }
  // односторонние ворота
  if(Math.random()<0.4+fav('gate')){ const c=take(); if(c) sp.set(c.k,{type:'gate',dir:pick(ORTHO)}); }
  // плита, открывающая соседнюю стену
  if(Math.random()<0.45+fav('plate')){ for(let tries=0;tries<10;tries++){ const c=take(); if(!c) break;
    const dirs=shuffle([...ORTHO]); let placed=false;
    for(const [dx,dy] of dirs){ const wx=c.x+dx, wy=c.y+dy;
      if(wset.has(key(wx,wy))){ sp.set(c.k,{type:'plate',opens:{x:wx,y:wy}}); placed=true; break; } }
    if(placed) break;
  } }
  // растекающаяся лава — один очаг
  if(Math.random()<0.35+fav('lava')){ const c=take(); if(c) sp.set(c.k,{type:'lava'}); }
  return sp;
}
...
export function buildFloorEnemies(flr){
  ...
  while(budget>=1 && bag.length<D.maxEnemies && guard++<100){
    ...
    else t = Math.random()<0.5 ? pick(aff) : aff.reduce((a,b)=>D.cost[b]>D.cost[a]?b:a, aff[0]);
    bag.push(t); budget-=D.cost[t];
  }
  while(bag.length<D.minEnemies) bag.push('pawn');
  return shuffle(bag);
}
export function enemyRangeBonus(flr){
  let b=0; if(flr>=CFG.DIFF.rangeBumpFloor) b++; if(flr>=CFG.DIFF.rangeBumpFloor2) b++;
...
export function spawnEnemiesForFloor(f, reach){
  ...
  const cand=[];
  for(let y=0;y<Math.ceil(CFG.H*0.62);y++) for(let x=0;x<CFG.W;x++){
    if(!reach.has(key(x,y))) continue;
    if(S.special.get(key(x,y))?.type==='trap'||S.special.get(key(x,y))?.type==='lava') continue;    // не спавнить на шипах/лаве
    if(Math.abs(y-S.player.y)<2 && Math.abs(x-S.player.x)<2) continue;
    cand.push({x,y});
  }
  shuffle(cand);
  const mk=(t,c)=>{
    const o={type:t, x:c.x, y:c.y, facing:[0,1], cd:0, status:{},
             homeColor:tileColor(c.x,c.y), r:(CFG.BASE_R[t]||1)+rb, rb};
  ...
  for(const t of bag){
    ...
    let idx=cand.findIndex(c=> !enemyAt(c.x,c.y) && !threatCellsFrom(mk(t,c), c.x, c.y).has(pk));
...
export function newFloor(){
  ...
  S.walls=room.walls; S.special=room.specials;
  S.player.x=room.playerStart.x; S.player.y=room.playerStart.y; S.player.facing=[0,-1];
  S.player.active=0;                          // спускаемся пешкой — иначе промоушен-петля обрывается
  S.promotionUsed=false; S.hoverEnemy=null; S.selectedEnemy=null; S.turn=1; S.player.freeSwapUsed=false; S.player.capturedThisFloor=0;
  ...
  if(S.player.nextFloorStatus&&S.player.nextFloorStatus.length){ // благословение алтаря
    S.player.nextFloorStatus.forEach(s=>applyStatus(S.player,s.k,s.n)); S.player.nextFloorStatus=[]; }
  spawnEnemiesForFloor(S.floor, room.reach);
  const rb=enemyRangeBonus(S.floor);
  log(`── Этаж ${S.floor} · ${S.biome.name} ── врагов: ${S.enemies.map(e=>GLYPH[e.type]).join(' ')||'—'}${rb?`  · дальность +${rb}`:''}`, 'e');
  render(); syncUI();
}

export function reset(){
  S.player = { x:0, y:0, facing:[0,-1], wheel:[ makeForm('pawn'), makeForm('knight'), null ], active:0,
             relics:new Set(), curses:new Set(), pawnShieldUsed:false, freeSwapUsed:false,
             capturedThisFloor:0, totalCaptures:0, status:{}, gold:0, nextFloorStatus:[] };
  S.unlocked = new Set(['pawn','knight']);
  S.gameOver=false; S.floor=0;
  if(dom.logEl) dom.logEl.innerHTML='';
  log('Новый забег. Зачисти этаж — выбираешь награду и спускаешься глубже, сохраняя формы и модификаторы.','');
  ...
  if(startRelics>0){ const pool=Object.keys(RELICS); shuffle(pool); pool.slice(0,startRelics).forEach(applyRelic); }
  newFloor();
}
