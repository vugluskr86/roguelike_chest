...
let _rt; window.addEventListener('resize',()=>{ clearTimeout(_rt); _rt=setTimeout(resizeBoard,80); });
...
function cellFromEvent(ev){
  ...
  return { x:Math.floor((cx-r.left)/(r.width/CFG.W)),
...
function handleTap(ev){
  ...
  const legal = moves.some(c=>c.x===x&&c.y===y)||captures.some(c=>c.x===x&&c.y===y);
  if(legal){ S.selectedEnemy=null; S.hoverEnemy=null; tryMoveTo(x,y); return; }
  const e=enemyAt(x,y);
...
document.addEventListener('keydown',ev=>{
  ...
  switch(ev.key.toLowerCase()){
    case 'q': case 'й': rotate(-1); break;
    case 'e': case 'у': rotate(1); break;
    case ' ': ev.preventDefault(); pass(); break;
    case '1': switchForm(0); break;
    case '2': switchForm(1); break;
    case '3': switchForm(2); break;
    case 'h': case 'р': case '?': openHelp(); break;
    case 'r': case 'к': closeModal(); reset(); break;
  }
});
...
document.getElementById('btnRestart').onclick=()=>{closeModal();reset();};
...
if(window.matchMedia && window.matchMedia('(hover:hover) and (pointer:fine)').matches){
  dom.cv.addEventListener('mousemove',ev=>{
    ...
    if(e!==S.hoverEnemy){ S.hoverEnemy=e; render(); }
  });
  dom.cv.addEventListener('mouseleave',()=>{ if(S.hoverEnemy){ S.hoverEnemy=null; render(); } });
}
