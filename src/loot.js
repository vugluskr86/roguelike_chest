...
export function rollWeighted(poolFn, n, used, biasHigh){
  ...
  for(let k=0;k<n && avail.length;k++){
    let total=0; const weights=avail.map(id=>{ const w=tierWeight(relicTier(id), S.floor, biasHigh); total+=w; return w; });
    let r=Math.random()*total, idx=0;
    while(idx<avail.length-1 && (r-=weights[idx])>0) idx++;
    const id=avail.splice(idx,1)[0]; got.push(id); used.add(id);
  }
  return got;
}
export function rollDistinct(poolFn, n, used){    // равновероятно (для проклятий)
  const avail=poolFn().filter(id=>!used.has(id));
  shuffle(avail);
  const got=avail.slice(0,n); got.forEach(id=>used.add(id)); return got;
}
...
export function buildLootOptions(){
  ...
  for(let i=0;i<2;i++){
    ...
    if(roll<0.45 && rLeft>=2 && cLeft>=1){
      opts.push({ kind:'faust', relics:rollWeighted(relicPool,2,usedR,true), curses:rollDistinct(cursePool,1,usedC) });
    } else if(roll<0.75 && rLeft>=3 && cLeft>=2){
      opts.push({ kind:'altar', relics:rollWeighted(relicPool,3,usedR,true), curses:rollDistinct(cursePool,2,usedC) });
    } else if(rLeft>=1){
      opts.push({ kind:'relic', relics:rollWeighted(relicPool,1,usedR,false), curses:[] });
    }
  }
...
export function offerLoot(){
  if(relicPool().length===0){ maybeEvent(); return; }   // все реликвии собраны — сразу к событию/спуску
...
export function applyRelic(id){
  ...
  if(id==='extra_slot'){ if(S.player.wheel.length<5) S.player.wheel.push(null); }   // +1 слот сразу
...
export function applyCurse(id){
  ...
  if(id==='rusted' && S.player.wheel.length>1){        // −1 слот: теряем последний (форма в нём уничтожается)
    S.player.wheel.pop();
