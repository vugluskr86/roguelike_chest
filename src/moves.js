...
export function genMoves(piece, form, isEnemyCell, isBlocked){
  ...
  const free=(x,y)=>inB(x,y)&&!S.walls.has(key(x,y))&&!isBlocked(x,y)&&!isEnemyCell(x,y);
  // ворота (только по стрелке) и цветовые зоны (только слон) действуют как стена
  const blk=(x,y,dir)=>{ const s=S.special&&S.special.get(key(x,y)); if(!s) return false;
    if(s.type==='colorzone') return form.type!=='bishop';
    if(s.type==='gate'){ if(!dir) return true; return !(dir[0]===s.dir[0]&&dir[1]===s.dir[1]); }
    return false; };
  // бонус дальности слайдеров: реликвии (только игрок) + ускорение (любая фигура)
  const reachBonus = (mine ? ((has('slider_reach')?1:0)
  ...
  const slide=(dirs,R)=>{
    for(const [dx,dy] of dirs){
      for(let s=1;s<=R;s++){
        ...
        if(isEnemyCell(x,y)){ captures.push({x,y}); break; }
  ...
  switch(form.type){
    case 'pawn': {
      ...
      if(free(mx,my) && !blk(mx,my,[fx,fy])){
        ...
        if((mine && has('pawn_double')) || hasteOn){       // Длинный шаг / ускорение: 2 клетки вперёд
    ...
    case 'bishop': {
      ...
      slide(DIAG, Math.max(1,(form.r??CFG.BASE_R.bishop)+bonus+reachBonus)); break;
    }
    case 'rook':  slide(ORTHO, Math.max(1,(form.r??CFG.BASE_R.rook)+reachBonus)); break;
    case 'queen': slide([...ORTHO,...DIAG], Math.max(1,(form.r??CFG.BASE_R.queen)+reachBonus)); break;
    case 'king': {
...
export function effectiveForm(e){
  if(e.type==='mimic'){
    ...
    return { type:t, r:(CFG.BASE_R[t]||1)+(e.rb||0)+(curse('mimic_reach')?1:0), homeColor:e.homeColor };
  }
...
export function enemyThreat(e){
  ...
  if(ef.type==='pawn'){
    ...
    const perp = fx===0 ? [[-1,fy],[1,fy]] : [[fx,-1],[fx,1]];
    perp.forEach(([dx,dy])=>{const x=e.x+dx,y=e.y+dy; if(inB(x,y)&&!S.walls.has(key(x,y))) set.add(key(x,y));});
    return set;
  }
  const {moves,captures}=genMoves(e, ef,
...
export function activeForm(){ return S.player.wheel[S.player.active]; }

export function playerOptions(){
  const f=activeForm();
  return genMoves(S.player, f, (x,y)=>!!enemyAt(x,y), ()=>false);
}
...
export function attackSquaresFor(e){
  const s=new Set(), pk=key(S.player.x,S.player.y);
...
export function movesToThreaten(e, sx, sy, attackSet, cap=5){
  ...
  let frontier=[{x:sx,y:sy}], d=0;
  while(frontier.length && d<cap){
    d++;
    const next=[];
    for(const c of frontier){
      const ox=e.x, oy=e.y; e.x=c.x; e.y=c.y;
      const {moves}=genMoves(e,effectiveForm(e),()=>false,(x,y)=>{const o=enemyAt(x,y);return !!o&&o!==e;});
      e.x=ox; e.y=oy;
      for(const m of moves){ const mk=key(m.x,m.y);
