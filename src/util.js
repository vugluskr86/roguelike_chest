...
export function makeForm(type, homeColor=0, improved=false){
  ...
  return { type, r: base + (improved && (type==='bishop'||type==='rook'||type==='queen') ? 1:0),
           improved, cooldown:0, homeColor };
}

export const randInt=n=>Math.floor(Math.random()*n);
export const pick=a=>a[randInt(a.length)];
export function shuffle(a){ for(let i=a.length-1;i>0;i--){ const j=randInt(i+1); [a[i],a[j]]=[a[j],a[i]]; } return a; }
