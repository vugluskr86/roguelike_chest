...
beforeEach(() => { reset(); Object.assign(META, defaultMeta()); S.player.relics.clear(); S.player.curses.clear(); S.player.status = {}; S.player.nextFloorStatus = []; S.modalOpen = false; });

describe('gold + event rooms', () => {
  it('enemies drop gold', () => {
    S.player.gold = 0; recordKill('queen', false);
    expect(S.player.gold).toBe(5);
  });
  it('shop: buy relic deducts gold, grants relic, unlocks merchant', () => {
    S.player.gold = 50; openShop();
    const g0 = S.player.gold, r0 = S.player.relics.size;
  ...
  it('purify removes a curse', () => {
    S.player.curses.add('brittle'); openPurify();
  ...
  it('sanctuary trades a form for a relic', () => {
    S.player.wheel = [makeForm('pawn'), makeForm('rook'), makeForm('knight')]; S.player.active = 1;
    S.player.relics.clear(); openSanctuary();
  ...
  it('gamble spends gold for one outcome', () => {
    S.player.gold = 20; S.player.relics.clear(); S.player.curses.clear(); openGamble();
    const r0 = S.player.relics.size, c0 = S.player.curses.size;
