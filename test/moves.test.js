...
import { genMoves, effectiveForm, enemyThreat, activeForm } from '../src/moves.js';
...
beforeEach(() => { reset(); S.walls = new Set(); S.special = new Map(); S.enemies = []; S.modalOpen = false; });

describe('genMoves', () => {
  it('rook slides orthogonally, blocked by walls', () => {
    S.player.x = 5; S.player.y = 5; S.walls = new Set([K(5, 2)]);
  ...
  it('color zone blocks non-bishop, passes bishop', () => {
    S.player.x = 5; S.player.y = 5; S.special = new Map([[K(5, 3), { type: 'colorzone' }]]);
  ...
  it('one-way gate passes along arrow only', () => {
    S.player.x = 5; S.player.y = 5; S.special = new Map([[K(5, 3), { type: 'gate', dir: [0, -1] }]]);
    let m = genMoves(S.player, { type: 'rook', r: 5 }, no, no);
    expect(has(m.moves, 5, 3)).toBe(true);
    S.player.y = 1;
