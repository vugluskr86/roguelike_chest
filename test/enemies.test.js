...
describe('special enemies', () => {
  it('guardian threatens like king (8 neighbours)', () => {
    const g = mkE({ type: 'guardian', x: 5, y: 5, armor: 2 });
    S.enemies = [g];
    S.player.x = 0;
    S.player.y = 0;
  ...
  it('necromancer summons a pawn, stays put', () => {
    S.player.x = 0;
    S.player.y = 0;
    const n = mkE({ type: 'necro', x: 5, y: 5, spawnCd: 0 });
    S.enemies = [n];
  ...
  it('priest shields adjacent ally', () => {
    S.player.x = 0;
    S.player.y = 8;
    const pr = mkE({ type: 'priest', x: 5, y: 5, priestCd: 0 });
    const ally = mkE({ type: 'pawn', x: 6, y: 5, cd: 99 });
    S.enemies = [pr, ally];
    enemiesTurn();
  ...
  it('frost mage stuns at range, does not threaten in melee', () => {
    ...
    S.player.y = 5;
    S.player.status = {};
    const fr = mkE({ type: 'frost', x: 5, y: 7, frostCd: 0 });
    S.enemies = [fr];
