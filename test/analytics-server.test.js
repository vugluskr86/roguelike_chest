...
describe('analytics server modules', () => {
  ...
  it('persists, lists and prunes isolated runs', async () => {
    ...
    try {
      await storage.init();
      await storage.writeMeta('test-run-0001', { runId: 'test-run-0001', schema: 1, startedAt: '2020-01-01T00:00:00.000Z', extra: { browserFingerprint: 'v1-device' } });
      await storage.appendEvents('test-run-0001', [{ n: 1, type: 'run_finished', state: { player: { x: 1, y: 2 } } }]);
      expect(await storage.readReplay('test-run-0001')).toMatchObject({ runId: 'test-run-0001', events: [{ type: 'run_finished', state: { player: { x: 1, y: 2 } } }] });
      await storage.writeReplay('test-run-0001', { runId: 'test-run-0001', events: [] });

      expect(await storage.listRuns()).toMatchObject([{ runId: 'test-run-0001', eventCount: 1, finished: true }]);
      expect(await storage.readReplay('test-run-0001')).toEqual({ runId: 'test-run-0001', events: [] });
