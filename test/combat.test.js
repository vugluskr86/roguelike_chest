...
beforeEach(() => { reset(); S.walls = new Set(); S.special = new Map(); S.enemies = []; S.gameOver = false; S.modalOpen = false; });

describe('degradation ladder', () => {
  it('losing a form drops a rung; pawn capture ends run', () => {
    S.player.wheel = [makeForm('pawn'), makeForm('rook'), null]; S.player.active = 1; S.player.status = {};
...
describe('guardian armor', () => {
  it('first hit bumps, second kills', () => {
    S.player.wheel = [makeForm('pawn'), makeForm('knight'), null]; S.player.active = 1; S.player.x = 5; S.player.y = 6; S.player.status = {};
...
describe('special tile triggers', () => {
  it('trap degrades player (persistent one-shot)', () => {
    S.player.wheel = [makeForm('pawn'), makeForm('rook'), null]; S.player.active = 1; S.player.x = 4; S.player.y = 4; S.player.status = {};
    S.special = new Map([[K(4, 4), { type: 'trap' }]]);
    triggerSpecialForPlayer();
    expect(S.player.wheel[1]).toBeNull();
  });
  it('conveyor pushes player', () => {
    S.player.x = 5; S.player.y = 5; S.special = new Map([[K(5, 5), { type: 'conveyor', dir: [1, 0] }]]);
    triggerSpecialForPlayer();
    expect(S.player.x).toBe(6);
  });
  it('plate opens linked wall', () => {
    S.walls = new Set([K(6, 4)]); S.player.x = 6; S.player.y = 5;
    S.special = new Map([[K(6, 5), { type: 'plate', opens: { x: 6, y: 4 } }]]);
    triggerSpecialForPlayer();
    expect(S.walls.has(K(6, 4))).toBe(false);
  });
  it('ice stuns player, tile persists', () => {
    S.player.x = 4; S.player.y = 4; S.player.status = {};
    const sp = new Map([[K(4, 4), { type: 'ice' }]]); S.special = sp;
    triggerSpecialForPlayer();
    expect(S.player.status.stun).toBe(1);
    expect(sp.has(K(4, 4))).toBe(true);
  });
  it('lava spreads (capped) and kills enemy on landing', () => {
    S.player.x = 5; S.player.y = 8; S.player.facing = [0, -1];
    S.player.wheel = [makeForm('pawn'), null, null]; S.player.active = 0;
    S.special = new Map([[K(5, 4), { type: 'lava' }]]);
    S.enemies = [mkE({ type: 'pawn', x: 5, y: 3, facing: [0, 1], cd: 0 })];
    enemiesTurn();
    expect(S.enemies.length).toBe(0);
    // spread
    S.enemies = []; S.player.x = 0; S.player.y = 0;
