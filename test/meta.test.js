...
import { META, defaultMeta, metaSave, metaLoad, buyUpgrade, upgradeCost,
         codexSeeEnemy, recordKill, unlockAch, endRunMeta, codexProgress, achProgress } from '../src/meta.js';
import { applyRelic, applyCurse } from '../src/loot.js';

// META is a live binding; reset its contents each test
function fresh() { Object.assign(META, defaultMeta()); S.player.relics.clear(); S.player.curses.clear(); }
beforeEach(() => { reset(); fresh(); S.modalOpen = false; });

describe('meta progression', () => {
  it('earns shards and records at end of run', () => {
    S.floor = 5; S.player.totalCaptures = 10;
  ...
  it('persists via localStorage', () => {
    META.shards = 42; META.upgrades.startSlots = 2; metaSave();
...
describe('codex + achievements', () => {
  it('bestiary unlocks on seeing the special trio; kills + toxin', () => {
    codexSeeEnemy('guardian'); codexSeeEnemy('necro');
    expect(META.achievements.bestiary).toBeFalsy();
    codexSeeEnemy('mimic');
    expect(META.achievements.bestiary).toBe(true);
    recordKill('rook', false); recordKill('bishop', true);
  ...
  it('progress counters count discovered entries', () => {
    codexSeeEnemy('pawn'); applyRelic('trophy'); unlockAch('deep');
