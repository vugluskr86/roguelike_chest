...
import { generateRoom, floodReach, buildFloorEnemies, spawnEnemiesForFloor, reset, newFloor } from '../src/board.js';
...
describe('biomes', () => {
  ...
  it('all biomes have palette + style + pools', () => {
    expect(BIOMES.every(b => b.light && b.dark && b.wallStyle && b.favorEnemies && b.favorTiles)).toBe(true);
  });
});
...
describe('generation connectivity (all wall styles)', () => {
  for (const id of ['halls', 'corridors', 'pylons']) {
    it(`${id}: promotion line reachable, border clear`, () => {
      ...
      let promoUnreach = 0, borderWall = 0;
      for (let i = 0; i < 400; i++) {
        const room = generateRoom();
        const reach = floodReach(room.walls, room.playerStart);
        let ok0 = false; for (let x = 0; x < CFG.W; x++) if (reach.has(K(x, 0))) { ok0 = true; break; }
        if (!ok0) promoUnreach++;
        for (let y = 0; y < CFG.H; y++) if (room.walls.has(K(0, y)) || room.walls.has(K(CFG.W - 1, y))) borderWall++;
      }
